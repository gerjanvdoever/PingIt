        ...
        public static void Seed(ModelBuilder modelBuilder)
        {
            ...
            for (int i = 1; i <= 16; i++)
            {
                ...
                users.Add(new User
                {
                    ...
                    FirstName = voornaam,
                    LastName = achternaam,
                    Email = $"{voornaam.ToLower()}.{achternaam.ToLower()}@wijkmail.nl",
            ...
            var incidents = new List<Incident>
            {
                ...
                new Incident { Id = 4, Title = "Losliggende stoeptegel", Description = "Mijn moeder is bijna gevallen door een losse tegel op de hoek van de Schoolstraat.", Latitude = 51.7499m, Longitude = 5.1331m, CreatedAt = now.AddDays(1), Status = IncidentStatus.Reported, Priority = PriorityLevel.Normal, Deadline = now.AddDays(21), CreatedByUserId = 10 },
                new Incident { Id = 5, Title = "Omgevallen boom", Description = "Er ligt een grote boom over het fietspad aan de Buitenkade.", Latitude = 51.7488m, Longitude = 5.1320m, CreatedAt = now.AddDays(1), Status = IncidentStatus.Registered, Priority = PriorityLevel.Emergency, Deadline = now.AddDays(1), CreatedByUserId = 11 },
                new Incident { Id = 6, Title = "Verdacht steegje mogelijke drugshandel", Description = "In dat steegje wordt sowieso gedealt. Iedere avond allemaal jongens met capuchons op op scooters, ik vertrouw dit echt niet.", Latitude = 51.7470m, Longitude = 5.1312m, CreatedAt = now.AddDays(2), Status = IncidentStatus.InProgress, Priority = PriorityLevel.High, Deadline = now.AddDays(7), CreatedByUserId = 12, HandledByUserId = 3, Notes = "Melding doorgestuurd naar politie." },
                new Incident { Id = 7, Title = "Foutgeparkeerd voertuig", Description = "Auto staat al dagen op de stoep bij het gemeentehuis.", Latitude = 51.7490m, Longitude = 5.1309m, CreatedAt = now.AddDays(3), Status = IncidentStatus.Resolved, Priority = PriorityLevel.Normal, Deadline = now.AddDays(21), CreatedByUserId = 7, HandledByUserId = 3, Notes = "Voertuig is weggesleept." },
                new Incident { Id = 8, Title = "Wateroverlast verstopte put", Description = "Na regen staat de straat voor het huis blank, put lijkt verstopt.", Latitude = 51.7503m, Longitude = 5.1351m, CreatedAt = now.AddDays(5), Status = IncidentStatus.Reported, Priority = PriorityLevel.High, Deadline = now.AddDays(7), CreatedByUserId = null },
                new Incident { Id = 9, Title = "Geluidsoverlast vliegtuig", Description = "Vliegtuigen vliegen steeds lager over, vooral 's avonds veel lawaai, ik heb dit al eerder aangegeven en er is nog steeds niks gedaan hoe kan dit???", Latitude = 51.7518m, Longitude = 5.1306m, CreatedAt = now.AddDays(1), Status = IncidentStatus.Reported, Priority = PriorityLevel.Low, Deadline = now.AddDays(42), CreatedByUserId = null },
                new Incident { Id = 10, Title = "Geluidsoverlast muziek", Description = "Die jeugd heeft ook geen respect voor rust he ze zitten de hele avond harde boem boem muziek te draaien en het staat veels te hard in mijn tijd mochten we niet eens naar buiten na 10 uur want dat was gevaarlijk ik weet dat nog goed toen gingen we binnen altijd spelletjes spelen wat een mooie tijden waren dat.", Latitude = 51.7520m, Longitude = 5.1345m, CreatedAt = now.AddDays(2), Status = IncidentStatus.Reported, Priority = PriorityLevel.Normal, Deadline = now.AddDays(21), CreatedByUserId = 13 },
